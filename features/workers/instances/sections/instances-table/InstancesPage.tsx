...
import { fetchWorkerTypes } from "@/features/workers/worker-types/api";
import {
  drainInstance,
  fetchInstances,
  stopInstance,
...
export default function InstancesPage() {
  ...
  useEffect(() => {
    fetchWorkerTypes()
      .then((types) =>
        setWorkerNames(Object.fromEntries(types.map((t) => [t.id, t.name]))),
      )
      .catch(() => {});
  }, []);
  ...
  const handleSweep = useCallback(async () => {
    try {
      const marked = await sweepStaleInstances();
      showToast(`Sweep complete — ${marked} marked dead.`);
      await load();
    } catch (cause) {

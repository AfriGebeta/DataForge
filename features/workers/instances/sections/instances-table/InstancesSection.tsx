            ...
            <tbody>
              {instances.map((inst) => (
                <tr key={inst.id}>
                  ...
                  <td>{workerNames[inst.worker_id] ?? inst.worker_id.slice(0, 8)}</td>
                  <td>{inst.queue}</td>
                  <td>
                  ...
                  <td>
                    {inst.status !== "DEAD" && inst.throughput_mpm != null
                      ? `${inst.throughput_mpm}/min`
                      : "—"}
                  </td>
                  <td>{inst.processed_count.toLocaleString()}</td>
                  <td style={{ color: "var(--text-muted)" }}>
                    {formatHeartbeat(inst.last_beat_at)}
                  </td>
                  <td>
                    {inst.status === "DEAD" ? (

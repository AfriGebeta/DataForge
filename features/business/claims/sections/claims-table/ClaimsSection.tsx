...
const STATUSES: ClaimStatus[] = ["PENDING", "IN_REVIEW", "APPROVED", "REJECTED", "REVOKED"];
...
export default function ClaimsSection({
  claims, placeNames, loading, statusFilter, onStatusFilter,
              ...
              {claims.map((claim) => (
                ...
                >
                  <td style={{ fontWeight: 500 }}>
                    {placeNames[claim.placeId] ?? <span style={{ color: "var(--text-muted)" }}>{`place #${claim.placeId}`}</span>}
                  </td>
                  <td style={{ color: "var(--text-secondary)" }}>
                    {claim.contactName ?? claim.claimEmail ?? "—"}
                  </td>

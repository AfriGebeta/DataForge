...
export default function RejectModal({ claim, onClose, onRejected }: Props) {
  ...
  const handleReject = async () => {
    ...
    setError(null);
    try {
      await rejectClaim(claim.id, reason ? { reason } : undefined);
      onRejected();
      onClose();
      setReason("");
    } catch (cause) {
      ...
      <div style={{ background: "var(--surface-2)", border: "1px solid var(--border-strong)", borderRadius: 12, width: 420 }}>
        ...
        <div style={{ padding: 18 }}>
          ...
          <p style={{ fontSize: 13, color: "var(--text-primary)", marginBottom: 14 }}>
            Reject the claim for <strong>{claim.placeName ?? claim.placeId}</strong>?
          </p>
          <div className="fg">
